...
function InteractiveHero() {
  ...
  return (
    <section
      ref={containerRef}
      className="relative min-h-[70vh] sm:min-h-[75vh] md:min-h-[88vh] flex items-center justify-center overflow-hidden"
    ...
    >
      ...
      <div className="relative z-10 text-center px-6">
        <span className="hero-reveal block text-xs uppercase tracking-[0.35em] text-muted-foreground mb-8 sm:mb-6">
          {t("hero.portfolio")}
        </span>
        <h1 className="font-heading text-5xl sm:text-6xl md:text-7xl lg:text-8xl tracking-tight leading-[0.95]">
          <span className="hero-reveal block">MARÍA BORONAT</span>
          <span className="hero-reveal-delay block mt-3 sm:mt-4 text-lg sm:text-xl md:text-2xl lg:text-3xl font-light opacity-70">
            {t("hero.subtitle")}
          </span>
        </h1>
      </div>
      ...
      <div className="hero-reveal-delay absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2">
        <span className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">{t("hero.scroll")}</span>
...
function VideoPortfolioSection() {
  const { t } = useLanguage()
  return (
    <ScrollReveal className="py-12 sm:py-16 md:py-24 px-6 md:px-12">
      <div className="max-w-5xl mx-auto">
        <h2 className="font-heading text-2xl sm:text-3xl md:text-4xl tracking-tight mb-6 md:mb-8 text-center">
          {t("home.miniPresentation")}
        </h2>
